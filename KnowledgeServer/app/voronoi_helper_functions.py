...
def fetch_artwork_metadata_batch(artwork_ids, db):
    ...
    Returns:
        dict of artwork_id -> metadata dict
    """
    import json
    ...
    for row in cursor:
        try:
            ...
            # Get thumbnail URL (prefer small or medium_rectangle)
            thumbnail_url = (
            ...
            # Get full-size URL (prefer large or normalized)
            full_url = (
            ...
            # Extract description and metadata from various sources
            description = ''
            year = None
            medium = ''
            
            # Try to get info from wikiart first, then artsy
            for source in ['wikiart', 'artsy']:
                if source in descriptions:
                    source_data = descriptions[source]
                    if not description and isinstance(source_data, dict):
                        description = source_data.get('description', '')
                    if not year and isinstance(source_data, dict):
                        year_str = source_data.get('date', '')
                        # Extract year from date string (e.g., "1907-1908" -> 1907)
                        if year_str:
                            try:
                                year = int(year_str.split('-')[0])
                            except:
                                pass
                    if not medium and isinstance(source_data, dict):
                        medium = source_data.get('medium', '')
            
            metadata[row['image_id']] = {
                'title': row['title'] or 'Untitled',
                'artist': artist_names[0] if artist_names else 'Unknown Artist',
                'artist_names': artist_names,  # Full list if needed
                ...
                'keywords': related_keywords[:10]  # Limit keywords for response size
            }
            
        except Exception as e:
            # If parsing fails for an artwork, still include basic info
